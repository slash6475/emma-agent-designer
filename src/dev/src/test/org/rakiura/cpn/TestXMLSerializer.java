...
public class TestXMLSerializer extends TestCase {
  ...
  Net net;
  Place p1, p2, sp1, sp2; // subnet places
  Transition t1;
  InputArc a1;
  OutputArc a2;
  ...
  public void testSimpleNetXMLDump() {
  	...
  	try { XmlUtil.writeXML(doc, System.out);	} catch (java.io.IOException e) {e.printStackTrace();}
  }

	public void testJavaDump() {
		// testing Java File
		//System.out.println (BasicNet.createJavaSource(this.net));
		ByteArrayOutputStream os = new ByteArrayOutputStream ();
  	Document doc = XMLSerializer.buildXML(this.net);
		try { XmlUtil.writeXML(doc, os);	} catch (java.io.IOException e) {e.printStackTrace();}
		Net netCopy = NetGenerator.createNet(os.toString());
		// do some net assert tests here
		try {
			os.close();
		}
